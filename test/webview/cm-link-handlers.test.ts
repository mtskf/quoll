...
function captureDispatch() {
  const calls: Array<{ doc: string; selection: { anchor: number; head: number } }> = [];
  const dispatch = vi.fn((tr: { state: EditorState }) => {
    calls.push({
      doc: tr.state.sliceDoc(),
      selection: {
...
describe("insertLink — allowlisted hrefs", () => {
  it("wraps the selection in `[text](href)` with an https URL", () => {
    const state = stateWith("see anchor here", EditorSelection.single(4, 10)); // "anchor"
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("https://example.com")({ state, dispatch })).toBe(true);
    expect(calls).toHaveLength(1);
    expect(calls[0]?.doc).toBe("see [anchor](https://example.com) here");
  });

  it("wraps the selection with an http URL", () => {
    const state = stateWith("anchor", EditorSelection.single(0, 6));
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("http://example.com")({ state, dispatch })).toBe(true);
    expect(calls[0]?.doc).toBe("[anchor](http://example.com)");
  });

  it("wraps the selection with a mailto URL", () => {
    const state = stateWith("anchor", EditorSelection.single(0, 6));
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("mailto:a@b.c")({ state, dispatch })).toBe(true);
    expect(calls[0]?.doc).toBe("[anchor](mailto:a@b.c)");
  });

  it("accepts a relative path", () => {
    const state = stateWith("anchor", EditorSelection.single(0, 6));
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("/relative/path")({ state, dispatch })).toBe(true);
    expect(calls[0]?.doc).toBe("[anchor](/relative/path)");
  });

  it("accepts a fragment-only href", () => {
    const state = stateWith("anchor", EditorSelection.single(0, 6));
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("#frag")({ state, dispatch })).toBe(true);
    expect(calls[0]?.doc).toBe("[anchor](#frag)");
  });
});

describe("insertLink — empty selection", () => {
  it("returns false without dispatching", () => {
    const state = stateWith("anchor", EditorSelection.single(3));
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("https://example.com")({ state, dispatch })).toBe(false);
    expect(calls).toHaveLength(0);
  });
});

// URL-allowlist contract: the same matrix the PM-era commands.test.ts pinned
// (test/editor/commands.test.ts:210-240), ported byte-for-byte so a refactor
// can't silently drop the check.
describe("insertLink — URL-allowlist contract", () => {
  const hostile: Array<[string, string]> = [
    ["javascript scheme", "javascript:alert(1)"],
    ["javascript with C0 bypass", "java\nscript:alert(1)"],
  ...
  for (const [label, href] of hostile) {
    it(`rejects ${label} without dispatching`, () => {
      const state = stateWith("anchor", EditorSelection.single(0, 6));
      const { calls, dispatch } = captureDispatch();
      expect(insertLink(href)({ state, dispatch })).toBe(false);
      expect(calls).toHaveLength(0);
    });
  }
});

// Markdown encoded-bypass contract: an href whose RAW form passes
// isAllowedUrl but whose post-decode form is unsafe.
// The PM-era insertLink did NOT cover this — the PM serializer would
// re-encode the URL as a mark attr, decoupling raw input from disk bytes.
// The CM-era insertLink writes the href verbatim into `(...)` and Lezer
// decodes it back when read, so the disk bytes themselves carry the
// bypass. insertLink REJECTS any href whose decode differs from the input.
describe("insertLink — Markdown encoded-bypass rejection", () => {
  const encodedBypass: Array<[string, string]> = [
    ["javascript with backslash colon", "javascript\\:alert(1)"],
    ...
    ["javascript via surrogate substitute (decoded NUL)", "javascript&#xD800;:alert(1)"],
  ];

  for (const [label, href] of encodedBypass) {
    it(`rejects ${label} without dispatching`, () => {
      const state = stateWith("anchor", EditorSelection.single(0, 6));
      const { calls, dispatch } = captureDispatch();
      expect(insertLink(href)({ state, dispatch })).toBe(false);
...
// Markdown-structural rejection: href and link-text bytes that would
// break the inline-link parse.
describe("insertLink — Markdown-structural rejection", () => {
  // Selection text containing `]`, `[`, `\`, `\n`, `\r`.
  it("rejects when link-text contains `]`", () => {
    // Doc: "abc]def", select positions 0..7 (full string) → contains `]`
    const state = stateWith("abc]def", EditorSelection.single(0, 7));
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("https://x")({ state, dispatch })).toBe(false);
    expect(calls).toHaveLength(0);
  });
  ...
  it("rejects when link-text contains a newline", () => {
    const state = stateWith("abc\ndef", EditorSelection.single(0, 7));
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("https://x")({ state, dispatch })).toBe(false);
    expect(calls).toHaveLength(0);
  });

  // href containing structural / whitespace / control bytes. The
  // `\s` char class in HREF_REJECT_RE covers space, tab, newline, CR,
  // form feed, vertical tab — explicit rows here pin each commonly-seen
  // bypass so a future regex tightening does not silently widen the
  // accept set.
  it("rejects when href contains a space", () => {
    const state = stateWith("anchor", EditorSelection.single(0, 6));
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("https://example.com/a b")({ state, dispatch })).toBe(false);
    expect(calls).toHaveLength(0);
  });

  it("rejects when href contains a tab", () => {
    const state = stateWith("anchor", EditorSelection.single(0, 6));
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("https://example.com/a\tb")({ state, dispatch })).toBe(false);
    expect(calls).toHaveLength(0);
  ...
  it("rejects when href contains a newline", () => {
    const state = stateWith("anchor", EditorSelection.single(0, 6));
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("https://example.com/a\nb")({ state, dispatch })).toBe(false);
    expect(calls).toHaveLength(0);
  });

  it("rejects when href contains `(`", () => {
    const state = stateWith("anchor", EditorSelection.single(0, 6));
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("https://example.com/a(b")({ state, dispatch })).toBe(false);
    expect(calls).toHaveLength(0);
  ...
  it("rejects when href contains `<` or `>`", () => {
    const state = stateWith("anchor", EditorSelection.single(0, 6));
    const { calls, dispatch } = captureDispatch();
    expect(insertLink("https://example.com/a<b")({ state, dispatch })).toBe(false);
    expect(insertLink("https://example.com/a>b")({ state, dispatch })).toBe(false);
