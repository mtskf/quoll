...
- Fix the Claude Code (`⌘⌥K`) and Codex (`⌘J`) context handoffs shipping stale line references when fired right after typing. Outbound edits are debounced by ~300ms, so a handoff invoked mid-window used to point Claude/Codex at file content that lacked your latest keystrokes; both handoffs now flush the pending edit to the host first — the same barrier the editor-switch already uses — so the reference always reflects what you just typed.
