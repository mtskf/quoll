...
// NOTE (uniform list-item gap, 2026-07-10): every renderable list-item marker
// line now receives `--quoll-list-item-gap` uniformly (see list-hang-indent.ts /
// fold/index.ts). The former per-boundary `listItemGetsVerticalGap` predicate
// (which packed tight consecutive siblings by dropping their gap) was removed —
// the user chose an equal loose/tight spacing, so `isRenderableListItem` is the
// single gate for both the content-line padding and the fold-gutter offset.
