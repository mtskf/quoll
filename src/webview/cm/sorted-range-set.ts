 ...
 * some sites mix decoration kinds, e.g. link-reveal / inline-mark-reveal feed
 * `Decoration.mark` and `Decoration.replace` together, so `startSide` is not
 * globally uniform). The `to` tie-break is inherited for a deterministic
 * pre-`add` order; it is not separately observable through the returned set —
 * `RangeSet` re-sorts equal-`from` ranges by `to` on read — so no test pins it
 * in isolation. Stability keeps items with an identical `[from, to]`
