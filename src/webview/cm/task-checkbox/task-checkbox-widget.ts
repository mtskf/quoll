...
//
// sameAs() (the base's `eq`) is keyed on (checked, from) only — body text
// changes don't force a re-render because the aria-label can stay
// slightly stale (it refreshes only when (checked, from) changes — i.e.
// on a toggle or on an edit that shifts the marker's byte offset; typing
// on the SAME line after the marker does NOT refresh it). Deliberate tradeoff:
...
// `patchDOM` reuses that element across widget instances: after a distant
// edit shifts this marker, CodeMirror builds a NEW widget, `sameAs()` returns
// false, and `patchDOM` re-points the reused DOM — but it cannot re-bind
...
// when the position moves, which `patchDOM` can do and a closure cannot.
...
// Shared by the mousedown and keydown listeners in render. Falling back to
...
// the listeners (render) and re-set on every reuse (patchDOM), so a miss is
...
export class CheckboxWidget extends QuollWidget {
  ...
  constructor(
    ...
    /** Trimmed body text for the accessible name; not part of sameAs(). */
  ...
  protected render(view: EditorView, signal: AbortSignal): HTMLElement {
    ...
    span.addEventListener(
      ...
      (event) => {
        ...
        // DOM swap (sameAs() returns false on checked-state change → CM
  ...
  protected patchDOM(
    ...
    // same-class widget as `from`. `sameAs()` keys on (checked, from). A checked
    ...
    // inspection-only, see `render`) and refresh aria-label per the widget's
