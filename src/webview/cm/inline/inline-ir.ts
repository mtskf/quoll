// The neutral CommonMark inline tokenizer + IR that BOTH the table-cell
// renderer (table/cell-render.ts) and the block-image widget (image/image-field.ts)
// consume. It owns the DOM-free half of the pipeline:
//
//   tokenize(raw) → Segment<CellLeaf>[]
//     → resolveInline(segments) → Resolved<CellLeaf>[]   [inline-emphasis.ts]
//
// exposed as `parseCellInline`, plus the `commonMarkAltText` string-content
// flattener built on the same IR. The DOM rendering (renderReadonly /
// renderCellInline) lives in table/cell-render.ts, which drives this IR.
//
// Living in a neutral `cm/inline/` module (rather than in table/) is what lets
// image/image-field.ts reuse `commonMarkAltText` WITHOUT importing from table/,
// dissolving the former table ⇄ image import cycle (the block-image widget and
// the table cell renderer both parse inline alt/label text identically).
//
...
// algorithm (nesting, rule of 3, `***triple***` splitting).
//
...
// browser resolves back into a JS URL → XSS. Routing both this tokenizer and the
// block-image widget through the one shared function keeps the two render gates
// from drifting. The URL-safety verdict is computed here and carried in the IR
// leaf; renderReadonly (in cell-render.ts) only reads it.
...
// dropping the leaf at runtime. Exported so cell-render.ts's renderReadonly
// reuses the one guard.
...
// image widget (`image/image-field.ts`) and the table-cell `<img>` renderer
